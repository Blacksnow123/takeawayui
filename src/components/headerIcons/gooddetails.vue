      ...
      <div class="food-wrapper">
        <div class="food-content">
          <div class="img-wrapper">
            <img class="food-img" :src="food.food_img"/>
            <span
              class="close-bt icon-close" @click="closeView"
            ></span>
            <!-- <img class="share-bt" src="/static/images/share.png"/>
            <img class="more-bt" src="/static/images/more.png"/> -->
          </div>
          <div class="content-wrapper">
            <h3 class="name">{{food.food_name}}</h3>
            <p class="saled">{{food.sell_count}}</p>
            <p class="price">
        ...
        <div class="rating-wrapper">
          ...
          <ul class="rating-content" >
            <li
              v-for="(comment,index) in foodComments"
              :key="index"
              class="comment-item"
            >
              <div class="comment-header">
                <img :src="comment.user_icon" v-if="comment.user_icon"/>
                <img src="/static/images/anonymity.png" v-if="!comment.user_icon"/>
              </div>
...
<script>
  ...
  export default {
    ...
    watch:{
      food(val, oldVal){//普通的watch监听
            // console.log("a: "+val.food_id);
          this.$ajax.post("foodComment", {food_id:val.food_id}).then(result => {
          // console.log(result.data.data);
          this.foodComments = result.data.data;
          // console.log(this.foodComments);
          });
        },
    },
