    ...
    <div class="zfb-pay" v-if="show">
        <div class="content">
            ...
            <div class="loading-wrap" v-if="payStatus !== 0">
                <div class="loading">
                    <!--加载图标-->
                    <img src="/static/images/password/loading.png" class="loading-ico" alt="" v-if="payStatus === 1">
                    <img src="/static/images/password/success.png" class="success-ico" alt="" v-if="payStatus === 2">
                    <!--加载文字-->
                    <p v-if="payStatus === 1">{{this.loadingText}}</p>
                    <p v-if="payStatus === 2">{{this.finishedText}}</p>                    
                </div>
            </div>
            ...
            <div class="pay-fail" v-if="isShowFail">
                <div class="pay-fail-lay">
                    <h3 class="title">{{this.failTip}}</h3>
                    <div class="btns">
    ...
    methods: {
        ...
        success () {
            return new Promise((resolve, reject) => {
                ...
                // 数据库插入order
                let foodlist=this.selectedFoods = this.$store.getters.getSelectFoods;
                let extra=this.$store.getters.getExtrainfo;
                let totalPrice=this.$store.getters.getTotalPrice;
                let location =this.$store.getters.getlocationInfo;
                let user=this.$store.getters.getUser;
                // foodlist.push(extra);
                // console.log(foodlist);
                this.$ajax.post("insertOrder", { foodList: foodlist,extraInfo:extra,totalPrice:totalPrice,locationInfo:location,user:user}).then(result => {
...
<style scoped>
    * {
    ...
    .content .ico-back {
        ...
        width: 40px;
        height: 40px;
        background: url(/static/images/password/back.png) no-repeat center;
    }
    ...
    .keyboard .delete {
        border-bottom: 0;
        background: url(/static/images/password/delete.png) no-repeat center #d1d5db;
    }
